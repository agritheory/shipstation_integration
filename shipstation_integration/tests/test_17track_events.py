...
from shipstation_integration.geocoding import _build_query, nominatim_geocode
...
# ---------------------------------------------------------------------------
# Group 1 — _parse_event_time
# ---------------------------------------------------------------------------


def test_parse_event_time_utc_z():
	assert _parse_event_time("2022-04-04T23:35:22Z") == "2022-04-04 23:35:22"
...
def test_parse_event_time_with_offset():
	...
	assert _parse_event_time("2022-04-04T16:35:22+07:00") == "2022-04-04 16:35:22"


# ---------------------------------------------------------------------------
# Group 2 — _build_query
# ---------------------------------------------------------------------------


def test_build_query_from_parts():
	assert _build_query({"city": "Ontario", "state": "CA", "country": "US"}) == "Ontario, CA, US"


def test_build_query_falls_back_to_location():
	assert _build_query({"location": "GASQUET, CA, US"}) == "GASQUET, CA, US"
...
def test_resolve_geocodes_when_no_api_coords():
	...
	with patch("frappe.get_hooks", return_value=["some.geocode.hook"]), patch(
		...
		lat, lon, source = _resolve_coordinates(address, enable_geocoding=True)
...
def test_resolve_skips_geocoding_when_disabled():
	...
	with patch("frappe.get_hooks") as mock_hooks:
		lat, lon, source = _resolve_coordinates(address, enable_geocoding=False)
...
def test_sync_appends_new_events():
	...
	_sync_tracking_events(tn_doc, _fixture_track_info(), enable_geocoding=False)
...
def test_sync_skips_event_without_time():
	...
	_sync_tracking_events(tn_doc, track_info, enable_geocoding=False)
